...
export function GuidedDraggingTool() {
  ...
  let partProperties = { layerName: 'Tool', isInDocumentBounds: false }
  let shapeProperties = { stroke: 'gray', isGeometryPositioned: true }
  /** @ignore */
...
GuidedDraggingTool.prototype.doDragOver = function () {
  ...
  let partItr = (this.copiedParts || this.draggedParts).iterator
  if (partItr.next()) {
    let part = partItr.key
...
GuidedDraggingTool.prototype.doDropOnto = function () {
  ...
  if (partItr.next()) {
    ...
    let snap = this.isGuidelineSnapEnabled && !e.shift
...
GuidedDraggingTool.prototype.showHorizontalMatches = function (part, guideline, snap) {
  let objBounds = part.locationObject.getDocumentBounds()
  let p0 = objBounds.y
  let p1 = objBounds.y + objBounds.height / 2
  let p2 = objBounds.y + objBounds.height

  let marginOfError = this.guidelineSnapDistance
  let distance = this.searchDistance
  // compares with parts (or location objects) within narrow vertical area
  let area = objBounds.copy()
  area.inflate(distance, marginOfError + 1)
  let otherParts = this.diagram.findObjectsIn(
    area,
    function (obj) {
      return obj.part
    },
  ...
  otherParts.each(function (other) {
    ...
    let otherBounds = other.locationObject.getDocumentBounds()
    let q0 = otherBounds.y
    let q1 = otherBounds.y + otherBounds.height / 2
    let q2 = otherBounds.y + otherBounds.height
  ...
  if (bestPart !== null) {
    let offsetX = objBounds.x - part.actualBounds.x
    let offsetY = objBounds.y - part.actualBounds.y
    let bestBounds = bestPart.locationObject.getDocumentBounds()
    // line extends from x0 to x2
    let x0 = Math.min(objBounds.x, bestBounds.x) - 10
    let x2 = Math.max(objBounds.x + objBounds.width, bestBounds.x + bestBounds.width) + 10
    // find bestPart's desired Y
    let bestPoint = new go.Point().setRectSpot(bestBounds, bestOtherSpot)
...
GuidedDraggingTool.prototype.showVerticalMatches = function (part, guideline, snap) {
  let objBounds = part.locationObject.getDocumentBounds()
  let p0 = objBounds.x
  let p1 = objBounds.x + objBounds.width / 2
  let p2 = objBounds.x + objBounds.width
  ...
  otherParts.each(function (other) {
    ...
    let otherBounds = other.locationObject.getDocumentBounds()
    let q0 = otherBounds.x
    let q1 = otherBounds.x + otherBounds.width / 2
    let q2 = otherBounds.x + otherBounds.width
  ...
  if (bestPart !== null) {
    ...
    let y0 = Math.min(objBounds.y, bestBounds.y) - 10
    let y2 = Math.max(objBounds.y + objBounds.height, bestBounds.y + bestBounds.height) + 10
